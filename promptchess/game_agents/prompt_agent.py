...
class SinglePromptUpdate(BaseModel):
    ...
    piece_type: str = Field(
        ..., description="The lowercase name of the piece type to update (e.g., 'pawn', 'knight')."
    )
    new_prompt: str = Field(..., description='The new concise prompt for the chosen piece type.')
    reasoning: str = Field(
        ..., description='Brief explanation for choosing this piece type and prompt.'
    )
...
class PromptAgent:
    ...
    def __init__(self, color: str, model: str, behaviour_file: Path | str | None = None):
        ...
        if behaviour_file:
            try:
                ...
                log_warning(
                    f'Behaviour file {behaviour_file} not found for {self.color} PromptAgent.'
                )
    ...
    async def decide_single_prompt_update(
        self, board: ChessBoard, current_prompts_self: Dict[str, str]
    ) -> Optional[SinglePromptUpdate]:
        """
        ...
        if self.agent is None or Runner is None:
            log_error(
                f"PromptAgent ({self.color}) cannot generate prompts; 'agents' library missing."
            )
            return None
        ...
        filtered_prompts_self = {
            ptype: current_prompts_self.get(ptype, 'N/A') for ptype in active_piece_types
        }
        ...
        agent_input = (
            ...
            + '\n'.join(
                [
                    f'- {ptype.capitalize()}: {prompt}'
                    for ptype, prompt in filtered_prompts_self.items()
                ]
            )
            + '\n\n'
        ...
        try:
            ...
            if not single_update:
                log_error(
                    f'PromptAgent ({self.color}) received no output or failed parsing for single update.'
                )
                return None
            ...
            if single_update.piece_type not in active_piece_types:
                log_warning(
                    f"PromptAgent ({self.color}) chose an inactive or invalid piece type: '{single_update.piece_type}'. Active: {active_piece_types}"
                )
                return None

            log_info(
                f"PromptAgent ({self.color}) successfully decided single update for '{single_update.piece_type}': '{single_update.new_prompt}'. Reasoning: {single_update.reasoning}"
            )
            return single_update

        except Exception as e:
            log_error(
                f'PromptAgent ({self.color}) failed during single prompt update decision: {e}',
                exc_info=True,
            )
            return None
