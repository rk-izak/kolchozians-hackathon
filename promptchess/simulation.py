...
async def run_simulation():
    ...
    while not game.is_game_over() and turn_count <= MAX_TURNS:
        ...
        if single_update:
            ...
            log_info(
                f"Updating prompt for {current_color} {single_update.piece_type}: '{single_update.new_prompt}'"
            )
            success = game.update_fraction_prompt(
                current_color, single_update.piece_type, single_update.new_prompt
            )
            if not success:
                log_warning(
                    f'Failed to apply prompt update for {current_color} {single_update.piece_type}.'
                )
        else:
            log_info(
                f'{current_color.capitalize()} Agent decided not to update any prompt this turn.'
            )
        ...
        if chosen_move:
            ...
            if success:
                ...
                log_error(
                    f"!!! Failed to apply legally chosen move '{chosen_move}': {message} - Simulation Halted !!!"
                )
